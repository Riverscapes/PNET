...
# Metagroup Field that will be used to group groups of plots
meta_group_field = arcpy.GetParameterAsText(1)
# The name of the metagroup (truncated)
meta_group_field_name = arcpy.GetParameterAsText(2)
# Group Field that will be used to group plots within their metagroup
group_field = arcpy.GetParameterAsText(3)
# The name of the group (truncated)
group_field_name = arcpy.GetParameterAsText(4)
# Fields from the field that we want to compare between categories
field_db_fields = parse_multistring(arcpy.GetParameterAsText(5))
...
def main():
    ...
    if is_csv(input_field_csv):
        meta_group_field, meta_group_field_name, group_field, group_field_name, field_db_fields\
            = read_field_csv(input_field_csv)
